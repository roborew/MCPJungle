...
- `mcpjungle__show_server_tools` — requires the `server` argument. Returns the canonical `<server-name>__<tool-name>` list for that single server, including descriptions and input schemas.

There is no `mcpjungle__invoke` meta-tool. Once the agent has the canonical name and schema, it invokes the upstream tool directly through the same proxy.

The recommended workflow is:

1. Call `mcpjungle__list_servers` to see which servers are available.
2. Pick a server and call `mcpjungle__show_server_tools` with `server=<name>` to fetch its tools.
3. Invoke the upstream tool by its canonical name (`<server>__<tool>`).
