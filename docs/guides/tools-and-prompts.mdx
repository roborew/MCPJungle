...
For agents that would be overwhelmed by a full tool listing, the gateway also exposes a **lazy discovery** variant at `/mcp?mode=lazy` and `/v0/groups/{name}/mcp?mode=lazy`. Instead of registering every enabled tool, it registers exactly two meta-tools:
...
There is no `mcpjungle__invoke` meta-tool. Once the client processes the tool-list change, it invokes the activated canonical tool directly through the same proxy. Other servers' tool schemas stay outside that session until selected.

The recommended workflow is:

1. Call `mcpjungle__list_servers` to see which servers are available.
2. Pick a server and call `mcpjungle__show_server_tools` with `server=<name>` to activate its tools for the current session.
3. Process `notifications/tools/list_changed`, then invoke the activated upstream tool by its canonical name (`<server>__<tool>`).

In **enterprise** mode, both meta-tools honour the client's server allow list, so an unauthorized server never appears in `mcpjungle__list_servers` and `mcpjungle__show_server_tools` rejects it with an error.
